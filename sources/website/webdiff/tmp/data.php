...
if($_REQUEST['submit']){
	...
	$temp=file_get_contents('./data.txt', FILE_USE_INCLUDE_PATH);
	...
	//TRUNC TABLES
	$sql = "TRUNCATE `domdata`;TRUNCATE `sessions`;TRUNCATE `session_users`;TRUNCATE `tests`;TRUNCATE `users`;";
	mysql_query($query);
	
	$query = "INSERT INTO tests (browser,sessid, user_agent) VALUES ('".'test_browser'."','".addslashes($sessId)."','".addslashes($_SERVER['HTTP_USER_AGENT'])."')";
	mysql_query($query);
