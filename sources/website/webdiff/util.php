...
function addDomDataToDB($data, $testid){
  ...
  while(sizeof($worklist) > 0){
    ...
    if($cnt>100){
    	...
    	$list = "";
        if(mysql_errno()){
	      echo "<h4 style='color:red'>There was an error while submitting your data :".mysql_errno().":".mysql_error()."; Query:".$q."</h4>";
		  log_error(array("mysql_errno"=>mysql_errno(), "mysql_error"=> mysql_error(), "query"=>$q));
	      return;
        }
    }
  }
